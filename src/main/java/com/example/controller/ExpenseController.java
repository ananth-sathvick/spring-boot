...
public class ExpenseController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping(path = "/findByCategory/{cid}") // returns a list of expenses logged under category with (category_id =
                                                // cid)
    ...
    @PreAuthorize("hasRole('ADMIN')")
    // returns a list of expenses logged under user with (user_id = cid) and
    // category with (category_id = cid)
    @GetMapping(path = "/findByUserCategory/{uid}/{cid}")
    public ResponseEntity<Iterable<Expense>> getByUser(@PathVariable("uid") Integer uid,
            @PathVariable("uid") Integer cid) {
    ...
    // creates a new expense under user with (user_id = cid) and category with
    // (category_id = cid) and returns the new expense
    @PostMapping(path = "/add/{uid}/{cid}")
    public ResponseEntity<Expense> addExpense(@RequestBody Expense expense, @PathVariable("uid") Integer uid,
    ...
    @PutMapping(path = "/update") // updates an existing expense with new the values and returns the updated
                                  // expense entity
